...
from train_baseline_knn import train
...
with mlflow.start_run():
    model, trainset, testset, data = train(
        dataset_path="/datasets/ratings.csv", rating_scale=(0.5, 5.0), min_k=1, k=40
    )
    ...
    mlflow.sklearn.log_model(
        model, f"model_KNNbasic_MovieLens-{len(data.df)}_rating_scale-{(0.5, 5.0)}_min-k-1_k-40"
    )

    mlflow.log_artifact("/tmp/data.csv")
    mlflow.log_artifact("scripts/train_baseline_svd.py")
